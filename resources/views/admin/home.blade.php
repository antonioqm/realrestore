  ...
  <caption><div class="container">
                <div class="level" style="height: 105px;">
                  ...
                  <div class="level-rigth">
                    <p class="field">
                      <a @click="modalProfessionalOpen" class="button is-success is-medium">
    ...
    <tr>
      ...
      <th><abbr title="Estado">UF</abbr></th>
...
@foreach($professionals as $professional)
    <tr style="{{$professional->trashed() ? 'opacity: .8; filter: grayscale(100%);' : ''}}">
      <th>{{$professional->id}}</th>
      <td>
          <a style="text-transform: uppercase;" class="is-info" href="#" title="{{$professional->name}}">
      ...
      <td>
        <div class="block">
          {{--  é nesse include que capturo o Evento de Click de Esclusão do Profissional--}}
          @include('admin.partials.btn-scape-php-to-js')
 ...
 <div class="container">{{ $professionals->links() }}</div>
              {{-- TABELA --}}
          </div>
        </div>
      </section>
...
<section class="section">
        ...
        <nav class="level">
          <div class="level-item has-text-centered">
            <div>
              ...
              <p class="title">{{$countAccredited}}</p>
            </div>
          </div>
          <div class="level-item has-text-centered">
            <div>
              <p class="heading">Profissionais</p>
              <p class="title">{{$countPF}}</p>
            </div>
          </div>
          <div class="level-item has-text-centered">
            <div>
              <p class="heading">Estabelecimentos</p>
              <p class="title">{{$countPJ}}</p>
            </div>
          </div>
          <div class="level-item has-text-centered">
            <div>
              <p class="heading">Estados</p>
              <p class="title">{{$countState}}</p>
            </div>
          </div>
          <div class="level-item has-text-centered">
            <div>
              <p class="heading">Cidades</p>
              <p class="title">{{$countCity}}</p>
            </div>
          </div>
        </nav>
      </div>
    ...
    {{-- level END--}}
{{-- Modal Delete Professional --}}
<delete-professional
  :name="showModalDeleteProfessionalName"
  :id="deleteProfessionalID"
  :showModal="showModalDeleteProfessional"
  @close="showModalDeleteProfessional=false"
  @delete="deleteProfessional(deleteProfessionalID)"
  v-if="showModalDeleteProfessional">
</delete-professional>
{{-- FIM Modal Delete Professional --}}

{{-- Modal de Criação de novos credenciados --}}
  @include('admin.partials.modal-create-professional')
{{-- FIM Modal de Criação de novos credenciados --}}
